...
from .database import get_database_connection, AIAnalysis, Resume
...
class AIResumeAnalyzer:
    ...
    def generate_pdf_report(self, analysis_result, candidate_name, job_role):
        ...
        try:
            ...
            # Add candidate name and job role
            info_data = [
            ...
            score_data = [
                ["Resume Score", f"{analysis_result.get('score', 0)}%"],
                ["ATS Score", f"{analysis_result.get('ats_score', 0)}%"]
            ]
            ...
            content.append(Paragraph("Key Strengths and Areas for Improvement", heading_style))
            content.append(Spacer(1, 0.1*inch))
            ...
            if strengths or weaknesses:
                ...
                for i in range(max_len):
                    ...
                    sw_data.append([strength, weakness])
    ...
    def answer_resume_question(self, resume_text: str, question: str) -> Dict[str, Any]:
        """Answer questions about the resume using AI"""
        if not resume_text:
            return {"error": "Resume text is required for analysis."}
        ...
        try:
            ...
            prompt = f"""
            You are an expert resume analyst. You will be given a resume and a question about it.
            Your task is to answer the question based on the information in the resume.
            If the information is not available in the resume, clearly state that.
            Be specific and provide relevant details from the resume to support your answer.
    ...
    def get_recent_analyses(self, limit: int = 5) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            analyses = session.query(AIAnalysis).order_by(AIAnalysis.created_at.desc()).limit(limit).all()
    ...
    def analyze_resume_with_anthropic(self, resume_text, job_description=None, job_role=None):
        """Analyze resume using Anthropic Claude (placeholder)"""
        # This is a placeholder method since we're not using Anthropic Claude
        return {
            "analysis": "Anthropic Claude analysis is not available.",
            "resume_score": 0,
            "ats_score": 0,
            "model_used": "Anthropic Claude"
        }